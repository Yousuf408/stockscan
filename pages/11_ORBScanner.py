...
# ─────────────────────────────────────────────────────────────────────────────
# SESSION STATE INIT (Reduced via loop)
# ─────────────────────────────────────────────────────────────────────────────
SESSION_DEFAULTS = {
...
# ─────────────────────────────────────────────────────────────────────────────
# CSS - WHITE THEME (Minified ~120 lines)
# ─────────────────────────────────────────────────────────────────────────────
WHITE_THEME_CSS = """
<style>
...
def format_pct(x):
    if pd.isna(x) or x is None: return "0.00%"
...
def safe_int(x):
    try:
        if pd.isna(x) or x <= 0 or x > 1e9: return 0
        return int(float(x))
    except: return 0
...
def get_tradingview_stocks():
    try:
        count, df = (Query().select('name','close','change','volume','relative_volume','market_cap_basic','sector','gap')
                     .set_markets('india')
                     .where(col('close') > HARDCODED_SETTINGS['price_min'],
...
def get_intraday_data_for_symbol(yahoo_ticker, period="5d", interval="5m"):
    try:
        ...
        if data.index.tz is None: data.index = data.index.tz_localize('UTC').tz_convert(IST)
        else: data.index = data.index.tz_convert(IST)
        return data
    except: return None

def calculate_ema_200(data_5min):
    if data_5min is None or len(data_5min) < 200: return None
    try:
...
def process_candle_data_for_symbol(base_ticker, yahoo_ticker):
    for suffix in ['.NS', '-NS', '']:
        ...
        if mask_first.sum() == 0:
            ...
            if mask_first.sum() == 0: first_candle = df_day.iloc[0]
            else: first_candle = df_day[mask_first].iloc[0]
        else: first_candle = df_day[mask_first].iloc[0]
        mask_second = (df_day.index.hour == 9) & (df_day.index.minute >= 20) & (df_day.index.minute <= 25)
        if mask_second.sum() == 0:
            if len(df_day) >= 2: second_candle = df_day.iloc[1]
            else: continue
        else: second_candle = df_day[mask_second].iloc[0]
        ...
        return {
            'high_9_15': float(first_candle['High']), 'low_9_15': low_9_15, 'open_9_15': open_9_15,
            'close_9_15': float(first_candle['Close']), 'open_9_20': float(second_candle['Open']),
            'high_9_20': float(second_candle['High']), 'low_9_20': float(second_candle['Low']),
            'close_9_20': float(second_candle['Close']), 'max_high_up_to_10_15': max_high,
            'hit_low_9_20_to_35': hit_low_9_20_to_35, 'breakout_9_30_to_9_45': breakout_9_30_to_9_45,
            'prev_close': prev_close, 'prev_high': prev_high, 'gap_percent_fallback': gap_percent_fallback,
            'yahoo_ticker': yahoo_ticker, 'data_date': today.strftime("%Y-%m-%d"),
            'ema_200_9_15': ema_200_9_15, 'ema_200_current': ema_200_current, '200 EMA': ema_status_9_15,
            'current_200_ema_status': ema_status_current, 'current_price': float(data['Close'].iloc[-1])
        }
    return None
...
def check_candle_conditions(df, tickers_list):
    ...
    for col in ['candle_9_15_high','candle_9_15_low','candle_9_20_open','candle_9_20_high','candle_9_20_low','candle_9_20_close',
                'max_high_up_to_10_15','hit_low_9_20_to_35','breakout_9_30_to_9_45','data_date','prev_close','prev_high',
                'gap_percent_fallback','open_gap_percent','passes_candle_check','candle_check_status','yahoo_ticker',
                'inside_9_15','ema_200_9_15','ema_200_current','200 EMA','current_200_ema_status','current_price','open_9_15']:
    ...
    for idx, row in df.iterrows():
        ...
        if base_ticker in candle_data:
            ...
            for key in ['high_9_15','low_9_15','close_9_15','open_9_15','open_9_20','high_9_20','low_9_20','close_9_20',
                        'max_high_up_to_10_15','hit_low_9_20_to_35','breakout_9_30_to_9_45','prev_close','prev_high',
                        'gap_percent_fallback','yahoo_ticker','data_date','ema_200_9_15','ema_200_current','200 EMA',
            ...
            cond = (data['close_9_20'] <= data['high_9_15'] and
                    (data['high_9_20'] <= data['high_9_15']) and (data['low_9_20'] <= data['high_9_15']) and
                    data['close_9_20'] < data['open_9_20'] and not data['hit_low_9_20_to_35'])
            if cond:
                ...
                reasons = []
                if data['close_9_20'] > data['high_9_15']: reasons.append('9:20 close > 9:15 high')
                if not ((data['high_9_20'] <= data['high_9_15']) and (data['low_9_20'] <= data['high_9_15'])): reasons.append('9:20 high/low not below 9:15 high')
                if data['close_9_20'] >= data['open_9_20']: reasons.append('9:20 candle not bearish')
                if data['hit_low_9_20_to_35']: reasons.append('Touched 9:15 low')
...
def load_stage1_data():
    with st.spinner("🔄 Loading market data..."):
        ...
        if df.empty:
            return {'df': pd.DataFrame(), 'valid': [], 'invalid': [], 'failed': [], 'rejected': rejected_gap.to_dict('records') if not rejected_gap.empty else [], 'total_count': count, 'filtered_count': 0, 'timestamp': datetime.now(IST)}
        df = df.sort_values('change', ascending=False)
        tickers_list = df['ticker'].tolist()
        df, valid, invalid, failed = check_candle_conditions(df, tickers_list)
        df = df.sort_values('change', ascending=False)
        return {'df': df, 'valid': df[df['passes_candle_check'] == True]['ticker'].tolist(), 'invalid': invalid, 'failed': failed,
                'rejected': rejected_gap.to_dict('records') if not rejected_gap.empty else [], 'total_count': count,
                'filtered_count': len(df), 'timestamp': datetime.now(IST)}
...
def check_auto_buy_conditions(row):
    ...
    if isinstance(current_price, str):
        try: current_price = float(current_price.replace('₹', '').replace(',', ''))
        except: return False, "Invalid Price format"
    if high_9_15 is None or pd.isna(high_9_15) or high_9_15 <= 0: return False, "No 9:15 High data"
    if current_price is None or pd.isna(current_price) or current_price <= 0: return False, "No current price data"
    required_price = high_9_15 * 1.0015
    if current_price <= required_price: return False, f"Price {current_price:.2f} <= 9:15 High + 0.15% ({required_price:.2f})"
...
def execute_auto_buy(display_df):
    ...
    if available.empty: return [], [], "No new stocks available"
    placed, failed, bought = [], [], []
    for _, row in available.iterrows():
        if st.session_state['auto_buy_bought_today'] >= st.session_state['auto_buy_max_stocks']: break
        symbol = row['Symbol']
...
def check_real_time_breakout(row):
    ...
    if high_9_15 is None or pd.isna(high_9_15) or high_9_15 <= 0: return False
    if current_price is None or pd.isna(current_price) or current_price <= 0: return False
...
with col1: st.markdown('<div class="page-title">🔍 Gap Screener <span>· Professional Trading Scanner</span></div>', unsafe_allow_html=True)
with col2:
    st.session_state['user_capital'] = st.number_input("💰", min_value=1000, max_value=10000000, value=int(st.session_state['user_capital']), step=1000, key="capital_input", label_visibility="collapsed")
with col3:
    st.session_state['num_parts'] = st.number_input("📊", min_value=1, max_value=10, value=int(st.session_state.get('num_parts', 4)), step=1, key="parts_input", label_visibility="collapsed")
with col4:
    st.button("🔄 Refresh", key="refresh_btn", use_container_width=True, on_click=lambda: st.session_state.update({'stage1_data': None, 'force_table_refresh': True}))
...
if st.session_state['stage1_data']:
    ...
    with fc1:
        if is_after_9_25: st.checkbox("📊 Inside 9:15", key="show_inside_only")
        else: st.info("⏳ 9:20 after 9:25 AM")
    with fc2:
        if breakout_status == 'before_9_30': st.info("⏳ Breakout after 9:30 AM")
        else: st.checkbox("⚡ Breakout 9:30-9:45", key="show_breakout_only")
    with fc3: st.checkbox("📏 Small Candle (≤ 1.5%)", key="show_small_candle")
    with fc4: st.checkbox("🌙 AMO", key="amo_mode")
    with fc5: st.checkbox("🤖 Auto-Buy", key="auto_buy_enabled")
    with fc6:
        ...
        else: st.markdown('<span style="color:#888;font-size:0.7rem;">⚪ Auto-Buy OFF</span>', unsafe_allow_html=True)
    ...
    if display_df.empty:
        ...
        # ─── Columns & Formatting ───
        cols = ['name','close','change','gap','volume','relative_volume','inside_9_15','breakout_9_30_to_9_45','200 EMA','MaxQty','sector','high_9_15','current_price','close_9_15','ema_200_9_15','ema_200_current','current_200_ema_status','prev_high']
        display_df = display_df[[c for c in cols if c in display_df.columns]].copy()
        display_df.rename(columns={'name':'Symbol','close':'Price','change':'Chg%','gap':'Gap%','volume':'Volume','relative_volume':'Rel Vol','inside_9_15':'Inside 9:15','breakout_9_30_to_9_45':'Breakout','MaxQty':'MaxQty','sector':'Sector','high_9_15':'high_9_15','current_price':'current_price','prev_high':'Prev Day High'}, inplace=True)
        ...
        def get_breakout_display(row):
            ...
            if bs == 'before_9_30': return "⏳ Waiting"
            if bs == 'live_checking':
                h = row.get('high_9_15', 0); cp = row.get('current_price', 0)
        ...
        display_df['200 EMA'] = display_df.apply(lambda r: f"🟢 ₹{r['ema_200_9_15']:,.2f}" if pd.notna(r['ema_200_9_15']) and r['close_9_15'] > r['ema_200_9_15'] else (f"🔴 ₹{r['ema_200_9_15']:,.2f}" if pd.notna(r['ema_200_9_15']) else "⚪ N/A"), axis=1)

        def auto_buy_status(r):
            if r['Inside 9:15'] != '✅': return '❌ Not Inside'
            if r['current_200_ema_status'] != 'ABOVE': return '❌ Below EMA'
            h = r.get('high_9_15'); cp = r.get('Price')
            if isinstance(cp, str):
                try: cp = float(cp.replace('₹', '').replace(',', ''))
                except: cp = None
            if h is None or pd.isna(h) or h <= 0 or cp is None or pd.isna(cp) or cp <= 0: return '❌ No Price'
            required = h * 1.0015
        ...
        display_df.index += 1
        final_cols = ['Symbol','Price','Chg%','Gap%','Volume','Rel Vol','Inside 9:15','Breakout','200 EMA','9:15 High','Prev Day High','Auto-Buy Status','MaxQty','Sector']
