...
with left_col:
    ...
    if not watchlist:
        ...
        # Save updated statuses back to watchlist
...
with right_col:
    if not st.session_state.selected_symbol:
        ...
        tv_symbol  = get_tv_symbol(st.session_state.selected_symbol,
        ...
        st.markdown(
            ...
            f'font-size:18px;color:#0f1117;">{st.session_state.selected_symbol}</span>'
            ...
            f'<span style="font-size:12px;color:#7a8394;">TradingView Chart</span>'
            f'</div>',
            unsafe_allow_html=True
        )
        ...
        <div id="tradingview_chart"
             style="height:650px;width:100%;border-radius:10px;
                    overflow:hidden;border:1px solid #e0e3e8;"></div>
