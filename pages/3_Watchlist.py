...
# ══════════════════════════════════════════
#   STORAGE CORE FILE READ/WRITE VECTORMAP
# ══════════════════════════════════════════
...
def load_price_cache() -> dict:
    """Reads engine data from price_cache.json framework directly"""
...
def update_single_stock_cache(symbol: str, exchange: str, price: float, source: str):
    """Directly patches instant clicks into the core registry file"""
    try:
        cache_structure = {"mode": "offline", "last_update": now_ist().strftime("%H:%M:%S"), "stocks": {}}
        if os.path.exists(PRICE_CACHE_FILE):
            with open(PRICE_CACHE_FILE, "r") as f:
                cache_structure = json.load(f)
        
        if "stocks" not in cache_structure:
            cache_structure["stocks"] = {}
            
        cache_structure["stocks"][symbol] = {
            "price": float(price),
            "source": source,
        ...
        print(f"Error patching price matrix on submission: {e}")


# ══════════════════════════════════════════
#   DIRECT PRICE FETCH WRAPPERS
# ══════════════════════════════════════════
...
def fetch_price_angel(symbol: str, exchange: str):
    try:
        ...
        resp  = obj.ltpData("NSE" if exchange == "NS" else "BSE", symbol, str(token))
...
STATUS_LABEL = {
    "WATCHING": "Watching", "NEAR": "Near Entry", "TRIGGERED": "Entry Triggered",
    "SL_HIT": "SL Hit", "TARGET1": "T1 Hit", "TARGET2": "T2 Hit"
}
STATUS_COLOR = {
    "WATCHING": "#f59e0b", "NEAR": "#f59e0b", "TRIGGERED": "#00a854",
    "SL_HIT": "#e53935", "TARGET1": "#2563eb", "TARGET2": "#7c3aed"
}
...
SOURCE_ICON = {
    "websocket": "🟢", "http": "🟡", "yfinance": "🟠", "close_price": "🟠", "offline": "⚪"
}
...
with left_col:
    ...
    if st.session_state.show_add_form:
        with st.expander("➕ ADD TRADE", expanded=True):
            ...
            st.markdown("**Entry**")
            entry_val = st.number_input("Entry Price", value=st.session_state.f_entry, min_value=0.0, format="%.0f", key="add_entry", label_visibility="collapsed")
            
            sl_col, t1_col = st.columns(2)
            with sl_col:
                st.markdown("**SL**")
                sl_val = st.number_input("Stop Loss", value=st.session_state.f_sl, min_value=0.0, format="%.0f", key="add_sl", label_visibility="collapsed")
            with t1_col:
                st.markdown("**T1**")
                t1_val = st.number_input("Target 1", value=st.session_state.f_t1, min_value=0.0, format="%.0f", key="add_t1", label_visibility="collapsed")
            
            st.markdown("**T2** (optional)")
            t2_val = st.number_input("Target 2", value=st.session_state.f_t2, min_value=0.0, format="%.0f", key="add_t2", label_visibility="collapsed")
            st.markdown("**Notes** (optional)")
            note_val = st.text_input("Notes", placeholder="e.g. Breakout", key="add_note", label_visibility="collapsed")

            if st.button(f"{'▲ ADD LONG' if st.session_state.direction=='BUY' else '▼ ADD SHORT'} → {st.session_state.current_tab}", use_container_width=True, type="primary", key="add_submit"):
                if not symbol:
                    ...
                    dup = [s for s in lst if s.get("symbol")==symbol and s.get("exchange")==st.session_state.exchange and s.get("direction")==st.session_state.direction]
                    if dup:
                        st.error(f"⚠ {symbol} {st.session_state.direction} already exists")
                    else:
                        lst.append({
                            "symbol": symbol, "exchange": st.session_state.exchange,
                            "direction": st.session_state.direction, "entry": entry_val, 
                            "sl": sl_val if sl_val > 0 else None, "target1": t1_val if t1_val > 0 else None,
                            "target2": t2_val if t2_val > 0 else None, "note": note_val.strip() or None,
    ...
    for i, col in enumerate([tc1, tc2, tc3]):
        with col:
            ...
            if st.button(f"{'●' if is_active else ''}{WATCHLIST_NAMES[i]}({cnt})", key=f"tab_{WATCHLIST_NAMES[i]}", use_container_width=True, type="primary" if is_active else "secondary"):
    ...
    if st.session_state.sort_open:
        ...
        for btn, lbl in [(s1,"Default"),(s2,"Status"),(s3,"Symbol"),(s4,"Distance")]:
            with btn:
                key = lbl.lower()
                if st.button(lbl, use_container_width=True, key=f"sort_{key}", type="primary" if st.session_state.sort_by==key else "secondary"):
                    st.session_state.sort_by = key
                    st.rerun()

    # ── INTERFACE REFRESH LIFECYCLE ──
        ...
        for i, stock in enumerate(watchlist):
            ...
            if price:
                st.session_state.prices[key] = {
                    "price": price, "source": source, "time": now_ist().strftime("%H:%M:%S")
                }
    ...
    def sort_list(lst, by):
        ...
        if by == "symbol": return sorted(lst, key=lambda s: s.get("symbol",""))
        if by == "distance":
            def dist(s):
                sym_code = s.get('symbol','')
                key  = f"{sym_code}_{s.get('exchange','NS')}"
                data = st.session_state.prices.get(key) or master_file_cache.get(sym_code, {})
                ltp  = data.get("price")
                e    = s.get("entry")
    ...
    if not watchlist:
        ...
        for stock_idx, stock in enumerate(watchlist):
            ...
            if not price_data and sym in master_file_cache:
                price_data = {
                    "price": master_file_cache[sym].get("price"),
                    "source": master_file_cache[sym].get("source", "close_price")
                }
            elif not price_data:
                price_data = {}
            ...
            if st.session_state.edit_idx==stock_idx and st.session_state.edit_tab==current_tab:
                ...
                card_html = (
                    f'<div style="padding:10px 12px;background:{selected_bg};border:{selected_border};'
                    f'border-left:4px solid {status_color};border-radius:8px;margin-bottom:6px;">'
                    f'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">'
                    f'<div style="display:flex;align-items:center;gap:6px;">'
                    f'<span style="font-family:monospace;font-weight:700;font-size:14px;color:#0f1117;">{sym}</span>'
                    f'<span style="font-size:9px;font-weight:700;color:{buy_color};">{"▲" if dirn=="BUY" else "▼"}</span>'
                    f'</div><span style="font-size:10px;font-weight:600;padding:2px 7px;border-radius:10px;'
                    f'background:rgba(25,63,155,0.08);color:{status_color};">{status_text}</span></div>'
                    f'<div style="display:flex;align-items:baseline;gap:8px;">'
                    f'<span style="font-family:monospace;font-weight:700;font-size:15px;color:#0f1117;">{ltp_display}</span>'
                    f'<span style="font-family:monospace;font-size:11px;color:{pct_color};">{pct_val}</span>'
                    f'<span style="font-size:10px;color:#7a8394;">{src_icon}</span></div>'
                    ...
                    f'<span style="font-size:8px;color:#534ab7;font-weight:600;">T2</span><span style="font-family:monospace;font-size:11px;font-weight:600;color:{"#534ab7" if t2 else "#aaa"};">{fmt(t2)}</span></div>'
                    f'</div></div>'
                )
                ...
                with btn_cols[0]:
                    ...
                    if st.button(btn_label, key=f"chart_{stock_idx}", use_container_width=True, type="primary" if is_selected else "secondary"):
                ...
                with btn_cols[1]:
                    if st.button("↺", key=f"rst_{stock_idx}", use_container_width=True, help="Reset"):
                        lst = get_list(current_tab)
                        lst[stock_idx]["status"], lst[stock_idx]["lastPrice"] = "WATCHING", None
                        set_list(current_tab, lst)
                        st.session_state.prices.pop(f"{sym}_{exchange}", None)
                        st.rerun()
                with btn_cols[2]:
                    if st.button("✏", key=f"edt_{stock_idx}", use_container_width=True, help="Edit"):
                        st.session_state.edit_idx, st.session_state.edit_tab = stock_idx, current_tab
                        st.rerun()
                with btn_cols[3]:
                ...
                if note:
                    st.markdown(f'<div style="font-size:10px;color:#7a8394;margin-left:4px;margin-top:-4px;margin-bottom:4px;">📝 {note}</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="font-size:10px;color:#7a8394">{mkt_label} · {now_ist().strftime("%I:%M %p IST")}</div>', unsafe_allow_html=True)
...
with right_col:
    if not st.session_state.selected_symbol:
        st.markdown(
            '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:680px;border:1px dashed #e0e3e8;border-radius:12px;background:#fafafa;">'
            '<div style="font-size:48px">📈</div><div style="font-size:16px;font-weight:600;color:#0f1117;margin-top:12px;">Select a stock to view chart</div>'
            '<div style="font-size:13px;color:#7a8394;margin-top:6px;">Click any stock card on the left</div></div>',
            unsafe_allow_html=True
        )
    else:
