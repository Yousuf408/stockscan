...
from styles import apply_styles, sidebar_brand, page_header, content_wrap_start, content_wrap_end
...
apply_styles()
sidebar_brand()
page_header("Sector Performance", "📊", refresh_key="sector_refresh")
content_wrap_start()
...
if not data:
    st.error("⚠️ Could not fetch sector data. Click ⟳ to retry.")
    content_wrap_end()
    st.stop()
...
rows_html = ""
for s in data:
    bar_w = (abs(s['change']) / max_chg) * 84
    color = "#00a854" if s['direction'] == 'up' else "#e53935"
    bg    = "#f6fcf9" if s['direction'] == 'up' else "#fff8f8"
    sign  = "+" if s['change'] >= 0 else ""
    rows_html += f"""
    <div style="display:grid;grid-template-columns:96px 1fr 72px;align-items:center;
    gap:14px;padding:9px 16px;border-bottom:1px solid #f0f2f5;background:#fff;
    transition:background 0.15s;" onmouseover="this.style.background='{bg}'"
...
.footer {{
    padding:8px 16px; font-size:10px; color:#7a8394;
    text-align:right; border-top:1px solid #f0f2f5;
    font-family:'Inter',sans-serif; background:#fafbfc;
}}
</style></head><body>
<div class="metrics">
  <div class="card">
  ...
  <div class="card">
    ...
    <div class="value">
      ...
      <span style="color:#cdd1d8;font-size:12px;"> / </span>
...
<div class="chart-wrap">
  {rows_html}
  <div class="footer">
    ⟳ &nbsp;Updated: {updated} &nbsp;·&nbsp; Auto-refresh every 30s &nbsp;·&nbsp; Source: Yahoo Finance / NSE
  </div>
</div>
</body></html>"""
