    ...
    for name, token, kind in STOCKS_WATCHLIST:
        ...
        if ltp > 0:
            ...
            vol_ratio, vol_signal, status = calculate_volume_metrics(
                name, current_volume, change_pct, all_volumes
            )
    ...
    if not st.session_state.angel_connected:
        if st.button("🔌 Connect Angel One", use_container_width=True):
            with st.spinner("Logging in to Angel One..."):
                # ── Session caching — reuse existing login if available ──
                if "angel_auth" not in st.session_state:
                    st.session_state["angel_auth"] = angel_login()
                creds = st.session_state["angel_auth"]
...
if st.session_state.angel_connected:
    ...
    with st.expander("🔍 Debug Panel", expanded=False):
        ...
        st.json(dict(list(ticks_debug.items())[:10]))

    # ── Live Table ────────────────────────────────────────────
    st.subheader(f"📉 Falling Stocks — Live ({len(STOCKS_WATCHLIST)} watchlist)")

    # ── Fetch volumes ONCE into session_state ─────────────────
    if "lf_volumes" not in st.session_state:
        with st.spinner("Loading historical volumes..."):
            st.session_state["lf_volumes"] = get_all_volumes_batch()
        st.caption(f"✅ Loaded volume data for {len(st.session_state['lf_volumes'])} stocks")

    all_volumes = st.session_state["lf_volumes"]

    # ── Fragment — refreshes every 2 seconds, page stays stable
    ...
    def live_table():
        ...
        for name, token, kind in STOCKS_WATCHLIST:
            ...
            if tick and ltp > 0:
                vol_ratio, vol_signal, status = calculate_volume_metrics(
                    name, int(volume), change_pct, all_volumes
                )
                ltp_str   = f"₹{ltp:.2f}"
            ...
            rows.append({
                ...
                "Status"   : status,
                "Time"     : time_str,
                "_chg_pct" : change_pct,   # raw value for filter
            })

        df = pd.DataFrame(rows)

        # ── Filter: sirf negative change % wale stocks ────────
        df = df[df["_chg_pct"] < 0].drop(columns=["_chg_pct"]).reset_index(drop=True)

        # ── Sort: sabse zyada girne wale upar ─────────────────
        df["_sort_key"] = df["Signal"].apply(signal_sort_key)
        df = df.sort_values("Change %").drop(columns=["_sort_key"]).reset_index(drop=True)

        st.dataframe(df, hide_index=True, use_container_width=True)
        st.caption(
            f"📉 Showing {len(df)} falling stocks | "
