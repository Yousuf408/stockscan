...
# ── Signal cutoff time — stocks signaling AFTER this time are excluded ──
SIGNAL_CUTOFF_TIME = "10:00:00"   # HH:MM:SS in IST — change here if needed
...
def scanner_table():
    ...
    for _, row in df.iterrows():
        ...
        if symbol not in signal_data:
            # ── New signal — only save if BEFORE cutoff time ──
            # (Stocks signaling after cutoff are still tracked in
            #  Supabase via this save, but excluded from display below)
    ...
    # ── TIME CUTOFF FILTER — exclude stocks that first signaled  ──
    # ── AFTER SIGNAL_CUTOFF_TIME from the DISPLAY table only.    ──
    # ── They remain saved in Supabase (saved above) for records. ──
