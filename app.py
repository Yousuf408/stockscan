...
#   VERSION: 3.2.1 (Auto-Sync Hotfix)
# ══════════════════════════════════════════
        ...
        while True:
            try:
                ...
                if not market_on:
                    ...
                    # HOTFIX: Break sleep loop if user added a new stock to the watchlist file
                    mismatch_detected = len(watchlist_stocks) != len(cache_stocks)
                    
                    if close_fetched_date != today or cache_empty or mismatch_detected:
                        print(f"[{t()}] Off-market sync triggered. Syncing cache structure...")
                    ...
                    time.sleep(5) # Reduced sleep resolution slightly for faster reactive addition parsing
                    continue
