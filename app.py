...
if st.session_state.obj:
    ...
    token_id = df_stocks[df_stocks["Stock"] == selected_stock].iloc[0]["Token"]
    ...
    if st.button("🔥 Place BUY Order"):
        try:
            ...
            # Yahan hum check karenge ki response kya hai
            order_res = st.session_state.obj.placeOrder(params)
            
            if order_res:
                st.success(f"🎉 Order Placed! ID: {order_res}")
            else:
                st.error("❌ Angel One API returned NO ID. Market shayad closed hai.")
